...
const DefaultLabelComponent = ({ textProps, x, y, label, formatter }) => {
    return (jsxRuntime.jsx(text.Text, { ...textProps, textAnchor: "start", x: x, y: y, children: formatter(label) }));
};
const DefaultValueComponent = ({ textProps, x, y, value, formatter }) => {
    return (jsxRuntime.jsx(text.Text, { ...textProps, textAnchor: "end", x: x, y: y, fontWeight: 500, children: formatter(value) }));
};
const AxisRenderer = ({ ticks, tickLabelProps, yOffset, labelPosition, valuePosition, data, labelFormatter, valueFormatter, LabelComponent = DefaultLabelComponent, ValueComponent = DefaultValueComponent, }) => {
    ...
    return ticks.map(({ from, formattedValue }, index) => {
        ...
        return (jsxRuntime.jsxs(group.Group, { children: [jsxRuntime.jsx(LabelComponent, { textProps: textProps, x: labelPosition, y: y, label: formattedValue, formatter: labelFormatter }), jsxRuntime.jsx(ValueComponent, { textProps: textProps, x: valuePosition, y: y, value: sum, formatter: valueFormatter, data: data, index: index })] }, index));
    });
};
/**
...
const getDefaultYOffset = (data, yScaleConfig, height, isMultiSeries) => {
    ...
    const groupScale = scale.scaleBand({
        domain: dataKeys,
...
const BarListChart = ({ data, width, height, options = {}, margin = {
    ...
    const chartOptions = react.useMemo(() => {
        ...
        return {
            ...
            valueFormatter: options.valueFormatter ?? (value => numberFormatters.formatNumberCompact(value)),
