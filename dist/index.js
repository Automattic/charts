...
import { AnimatedAreaSeries, AnimatedAreaStack, AreaSeries, Axis, BarGroup, BarSeries, DataContext, GlyphSeries, Grid, TooltipContext, XYChart, buildChartTheme, useEventEmitter } from "@visx/xychart";
...
const getPointSpacingInHours = (data) => {
	return data.reduce((spacing, datom) => {
		...
		return instants.reduce((seriesSpacing, time, index) => index === 0 ? seriesSpacing : Math.min(seriesSpacing, Math.abs(differenceInHours(time, instants[index - 1]))), spacing);
...
const GlobalChartsProvider = ({ children, theme, locale, timeZone }) => {
	...
	const providerTheme = useMemo(() => theme ? mergeThemes(defaultTheme, theme) : defaultTheme, [theme]);
	...
	const updateHiddenSeries = useCallback((chartId, update) => {
	...
	const setSeriesVisibility = useCallback((chartId, seriesLabel, visible) => {
		updateHiddenSeries(chartId, (current) => {
...
const getLegendItemAriaLabel = (text, value, visible, interactive) => {
	const accessibleText = value != null && value !== "" ? sprintf(_x("%1$s, %2$s", "legend item label and value", "jetpack-charts"), text, String(value)) : text;
	if (interactive) {
		if (visible) return sprintf(_x("%s: visible. Toggle visibility.", "visible interactive legend item", "jetpack-charts"), accessibleText);
		return sprintf(_x("%s: hidden. Toggle visibility.", "hidden interactive legend item", "jetpack-charts"), accessibleText);
	}
	if (visible) return;
...
const defaultRenderGlyph$1 = ({ key, ...props }) => /* @__PURE__ */ jsx(DefaultGlyph$1, {
	seriesKey: key,
	...props
}, key);
...
const XyChartTooltip = (props) => {
	...
	return /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsx("g", { ref: anchorRef }), tooltipContext?.tooltipOpen && /* @__PURE__ */ jsx(XyChartTooltipContent, {
		...props,
		tooltipContext,
		container
	})] });
...
const AccessibleTooltip = ({ renderTooltip, selectedIndex, tooltipRef, keyboardFocusedClassName, series = [], mode = "group", verticalCrosshairStyle, horizontalCrosshairStyle, ...props }) => {
	...
	return /* @__PURE__ */ jsx(XyChartTooltip, {
		...props,
...
const getAllHiddenMessage = (interactive, itemType) => {
	if (itemType === "segments") {
		if (interactive) return _x("All segments are hidden. Click legend items to show data.", "chart empty state: interactive segments", "jetpack-charts");
		return _x("All segments are hidden.", "chart empty state: segments", "jetpack-charts");
	}
	if (interactive) return _x("All series are hidden. Click legend items to show data.", "chart empty state: interactive series", "jetpack-charts");
...
const WholeNumberTicks = ({ axis, numTicks, enabled, children }) => {
	...
	return /* @__PURE__ */ jsx(Fragment$1, { children: children(tickValues) });
};
//#endregion
...
const LineChartAnnotation = ({ datum, title, subtitle, subjectType = "circle", styles: datumStyles, testId, renderLabel, renderLabelPopover }) => {
	...
	const styles = useMemo(() => deepmerge.all([
		ANNOTATION_POINTERS,
...
const TooltipDate = ({ date, displayResolution }) => {
	...
	const format = useMemo(() => createDateFormatter(TOOLTIP_FORMAT_BY_RESOLUTION$1[displayResolution], formatting), [displayResolution, formatting]);
...
const LineChartInternal = forwardRef(({ data, chartId: providedChartId, width, height, className, margin, withTooltips = true, withTooltipCrosshairs, showLegend = false, legend = {}, renderGlyph = defaultRenderGlyph, glyphStyle = {}, withLegendGlyph = false, withGradientFill = false, smoothing = true, curveType, renderTooltip = renderDefaultTooltip, tooltipPlacement, tooltipStyle, withStartGlyphs = false, withEndGlyphs = false, animation, options = {}, onPointerDown = void 0, onPointerUp = void 0, onPointerMove = void 0, onPointerOut = void 0, onDatumActivate = void 0, zoomable = false, rescaleYOnVisibilityChange = true, defaultHiddenSeries, children, gridVisibility, gap = "md" }, ref) => {
	...
	const isSeriesVisible = useCallback((seriesLabel) => !hiddenSeries.has(seriesLabel), [hiddenSeries]);
	...
	const hasWholeNumberValues = useMemo(() => hasOnlyWholeNumbers(dataSorted.filter((series) => isSeriesVisible(series.label))), [dataSorted, isSeriesVisible]);
	...
	const bucketInfo = useMemo(() => getBucketInfo(dataSorted.filter((series) => isSeriesVisible(series.label)), options?.axis?.x?.tickResolution), [
		dataSorted,
		isSeriesVisible,
							...
							children: [
								...
								/* @__PURE__ */ jsx(WholeNumberTicks, {
									...
									children: (tickValues) => /* @__PURE__ */ jsxs(Fragment$1, { children: [
										...
										!allSeriesHidden && chartOptions.axis.y.display && /* @__PURE__ */ jsx(Axis, {
											...chartOptions.axis.y,
											...tickValues ? { tickValues } : {}
										})
									] })
...
const AreaChartInternal = forwardRef(({ data, chartId: providedChartId, width, height, className, margin, withTooltips = true, withTooltipCrosshairs, showLegend = false, legend = {}, stacked = true, stackOffset = "none", smoothing = true, curveType, fillOpacity, withStroke, renderTooltip = renderDefaultTooltip, animation, options = {}, onPointerDown, onPointerUp, onPointerMove, onPointerOut, zoomable = false, rescaleYOnVisibilityChange = true, defaultHiddenSeries, children, gridVisibility, gap = "md" }, ref) => {
	...
	const hasWholeNumberValues = useMemo(() => (!stacked || stackOffset === "none") && hasOnlyWholeNumbers(dataSorted.filter((series) => isSeriesVisible(series.label))), [
		dataSorted,
		isSeriesVisible,
		stacked,
		stackOffset
	...
	const bucketInfo = useMemo(() => getBucketInfo(dataSorted.filter((series) => visibleLabels.has(series.label)), options?.axis?.x?.tickResolution), [
		dataSorted,
		visibleLabels,
		options?.axis?.x?.tickResolution
	]);
...
const ClassifiedBarSeries = ({ barClassName, primaryKeys, groupPadding, ...props }) => {
	...
	const groupScale = useMemo(() => createGroupScale(primaryKeys, bandScale?.bandwidth?.() ?? 0, groupPadding), [
		primaryKeys,
		bandScale,
		groupPadding
	]);
	const renderGlyph = useCallback(({ datum, x, y, color, onBlur, onFocus, onPointerMove, onPointerOut, onPointerUp }) => {
	...
	return /* @__PURE__ */ jsx(GlyphSeries, {
		...props,
		renderGlyph
	});
};
...
const BarChartInternal = ({ data, chartId: providedChartId, width, height, className, margin, withTooltips = false, showLegend = false, legend = {}, gridVisibility: gridVisibilityProp, renderTooltip, tooltipPlacement, tooltipAnchorTop, tooltipStyle, barClassName, options = {}, orientation = "vertical", withPatterns = false, showZeroValues = false, withBandHighlight = false, onBandHighlightChange, defaultHiddenSeries, animation, children, gap = "md", onPointerDown, onPointerUp, onDatumActivate }) => {
	...
	const hasWholeNumberValues = useMemo(() => hasOnlyWholeNumbers(dataSorted.filter(isSeriesRendered)), [dataSorted, isSeriesRendered]);
	...
	const renderDefaultTooltip = useCallback(({ tooltipData }) => {
		...
		if (comparisonEntry && comparisonDatum) return /* @__PURE__ */ jsxs("div", {
						...
						children: /* @__PURE__ */ jsxs(XYChart, {
							...
							children: [
								...
								!allSeriesHidden && /* @__PURE__ */ jsx(WholeNumberTicks, {
									...
									children: (valueTicks) => /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsx(Grid, {
								...
								barClassName ? /* @__PURE__ */ jsx("g", {
									className: "visx-bar-group",
									children: primaryEntries.map(({ series: seriesData, index }) => /* @__PURE__ */ jsx(ClassifiedBarSeries, {
										dataKey: seriesData.label,
										data: seriesData.data,
										xAccessor: chartOptions.accessors.xAccessor,
										yAccessor: chartOptions.accessors.yAccessor,
...
const ConversionFunnelChartInternal = ({ mainRate, changeIndicator, steps, loading = false, animation, className, chartId: providedChartId, height, style, renderStepLabel, renderStepRate, renderMainMetric, renderTooltip }) => {
	...
	const renderDefaultMainMetric = () => /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsx("span", {
		...
		className: clsx(conversion_funnel_chart_module_default["change-indicator"], conversion_funnel_chart_module_default[isPositiveChange ? "change-indicator--positive" : "change-indicator--negative"]),
	...
	if (!isDataValid) return /* @__PURE__ */ jsx(Stack, {
		...
		justify: "center",
		ref: setScopeNode,
		className: clsx("conversion-funnel-chart", conversion_funnel_chart_module_default["conversion-funnel-chart"], loading && conversion_funnel_chart_module_default["conversion-funnel-chart--loading"], className),
		style: {
			...style,
	...
	return /* @__PURE__ */ jsx(HeatmapContext.Provider, {
		...
		children: /* @__PURE__ */ jsx(ChartInstanceContext.Provider, {
			...
			children: /* @__PURE__ */ jsxs(ChartLayout, {
				...
				trailingContent: nonLegendChildren,
				gap,
				className: clsx("heatmap-chart", heatmap_chart_module_default["heatmap-chart"], className, { [heatmap_chart_module_default["heatmap-chart--height-capped"]]: heightCapped }),
				style: {
					width: width || void 0,
					height: height || void 0
				},
...
const BarWithLabel = ({ entry, withComparison, withOverlayLabel, primaryColor, secondaryColor, animation, isPrimaryVisible = true, isComparisonVisible = true }) => {
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			showComparisonBar && hasPreviousValue(entry) && /* @__PURE__ */ jsx("div", {
				className: clsx(leaderboard_chart_module_default.bar, { [leaderboard_chart_module_default["bar--animated"]]: animation }),
				style: {
...
const LeaderboardChartInternal = ({ data, chartId: providedChartId, width: propWidth, height: propHeight, withComparison = false, withOverlayLabel = false, primaryColor, secondaryColor, valueFormatter = defaultValueFormatter, deltaFormatter = defaultDeltaFormatter, animation, loading = false, fitRows = false, showLegend = false, legend = {}, legendLabels, gap = "md", className, style, children }) => {
	...
	if (!data || data.length === 0) return /* @__PURE__ */ jsx(ChartInstanceContext.Provider, {
		...
		children: /* @__PURE__ */ jsx(ChartLayout, {
			...
			legendElement: false,
			legendChildren,
			className: clsx("leaderboard-chart", leaderboard_chart_module_default.leaderboardChart, {
						...
						const rowCells = /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsx(Stack, {
							...
							children: [
								...
								showComparisonValue && /* @__PURE__ */ jsx(Text$1, {
									className: clsx(leaderboard_chart_module_default.deltaValue, deltaTrendClass),
									children: deltaFormatter(entry.delta)
								}),
								showComparisonPlaceholder && /* @__PURE__ */ jsxs(Text$1, {
									className: clsx(leaderboard_chart_module_default.deltaValue, leaderboard_chart_module_default.deltaPlaceholder, deltaTrendClass),
									children: [/* @__PURE__ */ jsx("span", {
										"aria-hidden": "true",
										children: "—"
									}), /* @__PURE__ */ jsx(component_default$1, {
										as: "span",
										children: hasPreviousPeriodValue ? __("Percentage change unavailable", "jetpack-charts") : __("No comparison data", "jetpack-charts")
									})]
								})
							]
						})] });
...
const PieChartInternal = ({ data, chartId: providedChartId, withTooltips = false, className, showLegend = false, legend = {}, width: propWidth, height: propHeight, size, animation, thickness = 1, padding = 0, gapScale = 0, cornerScale = 0, showLabels = true, legendValueDisplay = "percentage", children = null, tooltipOffsetX = 0, tooltipOffsetY = -15, renderTooltip = renderDefaultPieTooltip, gap = "md" }) => {
	...
	return /* @__PURE__ */ jsx(ChartInstanceContext.Provider, {
		...
		children: /* @__PURE__ */ jsx(ChartLayout, {
			...
			trailingContent: /* @__PURE__ */ jsxs(Fragment$1, { children: [htmlChildren, otherChildren] }),
								...
								children: (pie) => {
									return pie.arcs.map((arc, index) => {
										...
										return /* @__PURE__ */ jsxs("g", {
											...groupProps,
											children: [/* @__PURE__ */ jsx("path", { ...pathProps }), showLabels && hasSpaceForLabel && /* @__PURE__ */ jsxs("g", { children: [/* @__PURE__ */ jsx("rect", {
